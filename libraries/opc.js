...
class OPC {
  ...
  static select(variableNameOrConfig, options, value = null) {
    ...
    if (typeof variableNameOrConfig === "object") {
      ...
      options = variableNameOrConfig.options;
      value =
        variableNameOrConfig.value ?? value ?? options[Object.keys(options)[0]];
      label = variableNameOrConfig.label;
      description = variableNameOrConfig.description;
    } else {
      variableName = variableNameOrConfig;
  ...
  static bezier(functionName, anchors = null) {
    ...
    if (
      ...
      anchors.length === 0 ||
      typeof anchors[0] !== "object" ||
      !("pX" in anchors[0])
    ) {
      throw new Error(
        "OPC.bezier expects anchor array format: [{pX, pY, cX, cY}, ...]",
      );
    }
  ...
  static getEaseFunction(option) {
    ...
    if (typeof anchors === "string") {
      try {
        ...
        throw new Error("OPC.getEaseFunction: invalid JSON");
      }
    }
    if (!Array.isArray(anchors) || anchors.length < 2) {
      throw new Error(
        "OPC.getEaseFunction expects at least two anchors in format: [{pX,pY,cX,cY}, ...]",
      );
    }
    ...
    anchors = anchors.map((a) => {
      if (
        typeof a !== "object" ||
        !("pX" in a && "pY" in a && "cX" in a && "cY" in a)
      ) {
        throw new Error(
          "OPC.getEaseFunction anchor missing one of pX,pY,cX,cY",
        );
      }
      return {
        pX: +a.pX,
    ...
    if (anchors[0].pX !== 0 || anchors[anchors.length - 1].pX !== 1) {
      ...
      if (
        Math.abs(anchors[0].pX - 0) > 1e-9 ||
        Math.abs(anchors.at(-1).pX - 1) > 1e-9
      ) {
        throw new Error(
          "OPC.getEaseFunction: first pX must be 0 and last pX must be 1",
        );
      }
    }
    for (let i = 1; i < anchors.length; i++) {
      if (anchors[i].pX < anchors[i - 1].pX - 1e-12) {
        throw new Error(
          "OPC.getEaseFunction: pX values must be non‑decreasing",
        );
      }
    }
    ...
    return function ease(t) {
      ...
      // exact boundaries
      if (x <= segments[0].p1x) {
        OPC.callParentFunction("OPC_bezier_marker", {
          name: option.name,
          x: segments[0].p1x,
          y: segments[0].p1y,
        });
  ...
  static setOSC(server, port = null) {
    ...
    if (typeof window.OSC === "undefined") {
      ...
      script.src =
        "https://cdn.jsdelivr.net/npm/osc@2.4/dist/osc-browser.min.js";
